  ...
  <div class="flex flex-col gap-4">
    <section class="rounded-2xl border border-line bg-card p-4">
      ...
      <p class="mt-2 text-sm whitespace-pre-wrap text-txt-hi">
        {{ ticket.data.value?.functional.body ?? t('storyTab.nothingWritten') }}
      </p>
    </section>

    <section class="rounded-2xl border border-info bg-card p-4">
      <p class="font-mono text-[10px] tracking-[0.18em] text-info uppercase">
        {{ t('storyTab.twin') }}
      </p>
      <p class="mt-2 text-sm whitespace-pre-wrap text-txt-hi">
        {{ ticket.data.value?.tests?.body ?? t('storyTab.twinNotWritten') }}
      </p>
    </section>

    <section class="rounded-2xl border border-line bg-card p-4">
      <p class="font-mono text-[10px] tracking-[0.18em] text-txt-low uppercase">
        {{ t('storyTab.definitionOfDone') }}
      </p>
      <ul class="mt-2 flex flex-col gap-1.5">
        ...
        >
          ...
          />
          <span class="text-txt-mid">{{ t(`checkpoint.${step.name}`) }}</span>

...
<template>
  <section class="flex flex-col gap-4 rounded-2xl border border-line bg-card p-5">
    <div class="flex flex-wrap items-center gap-3">
      <h2 class="display-italic m-0 text-xl">{{ t('setting.templates') }}</h2>
      <EffectBadge section="templates" />
    </div>

    <p class="text-[12.5px] text-txt-low">{{ t('template.whatItControls') }}</p>
    ...
    <ul class="flex flex-col gap-2">
      ...
      >
        ...
        <span class="font-mono text-[10px] text-txt-low">v{{ template.version }}</span>
        <span v-if="template.isDefault" class="rounded-md border border-acc px-1.5 py-0.5 font-mono text-[9.5px] text-acc uppercase">{{
          t('template.byDefault')
        }}</span>
        <button
          type="button"
          class="ml-auto rounded-lg border border-line px-2 py-1 font-mono text-[10px] text-txt-mid uppercase hover:border-acc"
    ...
    <label class="flex flex-col gap-1 text-[12px] text-txt-mid">
      {{ t('template.jsonl') }}
      <textarea
        v-model="carried"
        rows="6"
    ...
    <div class="flex flex-wrap items-center gap-3">
      <button
        ...
        class="rounded-lg border border-acc bg-acc px-3 py-1.5 font-mono text-[10px] font-bold text-ink uppercase disabled:opacity-40"
        @click="importOne()"
      >
        {{ t('template.import') }}
      </button>
      <span v-if="imported" class="font-mono text-[10px] text-green uppercase">{{
        t('template.imported')
      }}</span>

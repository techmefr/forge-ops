...
const openStory = computed(
  () => planning.value.find((story) => story.id === openStoryId.value) ?? null,
)
...
function startBuilding(): Promise<void> {
  ...
  return guard(async () => {
    await board.send(`/api/stories/${storyId}/plan/accept`, 'POST')
    await Promise.all([ticket.reload(), stories.reload()])
  })
}
  ...
  <div class="grid h-full min-h-[600px] grid-cols-[300px_minmax(0,1fr)]">
    ...
    <section class="min-w-0 overflow-auto p-6">
      ...
      <template v-else>
        ...
        <section
          v-if="openStory?.state === 'plan_review'"
          class="mt-5 rounded-2xl border border-green bg-card p-4"
        >
          <p class="display-italic text-sm text-green">Le plan est sur la table</p>
          <p class="mt-1 text-xs text-txt-low">
            Accepter le plan fait entrer la story dans le dev, sur le kanban.
          </p>
          <button
            type="button"
            :disabled="busy"

...
## The question: Reka UI or shadcn-vue
...
Depend on **Reka UI directly**, skip shadcn-vue. Reasoning:
...
- Is the native `<select>` in `AppearanceSection.vue` good enough forever (simpler, accessible by default, zero dependency), or is a richer picker worth pulling in `Select`/`Combobox` for?
- Worth adding `Tooltip` now as a small, low-risk first Reka UI component to validate the integration, before touching `Tabs`/`Dialog`?
- Any objection to a new runtime dependency at all, given the project's current zero-UI-dependency stance?
